...
SELECT
  ...
  t19.DISCHARGEDISPOSITION,
  t19.ChemoInd,
  t19.ADMISSIONDATE,
  t19.DISCHARGEDATE,
  t19.MEDICALRECORDNUMBER
FROM
  ...
  LEFT JOIN (
              ...
              FROM (
                SELECT DISTINCT
                  t1.ADMISSIONFISCALYEAR,
                  t1.ADMISSIONDATE,
                  t1.DISCHARGEDATE,
                  t1.DISCHARGEDISPOSITION,
                  t1.MEDICALRECORDNUMBER,
                  ROW_NUMBER()
                  OVER (
                    PARTITION BY t1.MEDICALRECORDNUMBER
                    ORDER BY t1.ADMISSIONDATE DESC ) AS Ind
            ...
            ) t19 ON t13.PT_DFCI_MRN = t19.MEDICALRECORDNUMBER
...
WHERE --t1.transact_typ_cd = 1
    t1.BILL_PROV_ID = '1012901'
    AND EXTRACT(YEAR FROM t1.SVC_DTTM) = 2017
    AND EXTRACT(MONTH FROM t1.SVC_DTTM) = 12
    AND t1.CPT = '99205'
    AND t1.TRANSACT_ID IS NOT NULL
    AND t1.ORIG_REVRSE_TRANSACT_ID IS NULL
