...
FROM (SELECT
      ...
      FROM (SELECT
            ...
            FROM DARTEDM.F_MTHLY_PROV_SERVICE_RVU@DARTPRD RVU
              JOIN DARTEDM.D_PROV@DARTPRD PROV
                ON RVU.PROV_DIM_SEQ = PROV.PROV_DIM_SEQ) a11
        LEFT OUTER JOIN DARTEDM.D_CLIN_DEPT@DARTPRD a12
          ON (a11.CLIN_DEPT_DIM_SEQ = a12.CLIN_DEPT_DIM_SEQ)
        LEFT OUTER JOIN DARTEDM.D_PROV@DARTPRD a13
          ON (a11.PROV_DIM_SEQ = a13.PROV_DIM_SEQ)
        LEFT OUTER JOIN DARTEDM.D_PROC@DARTPRD a14
          ON (a11.PROC_DIM_SEQ = a14.PROC_DIM_SEQ)
        LEFT OUTER JOIN DARTEDM.D_CALENDAR@DARTPRD a15
          ON (a11.BEG_MTH_DIM_SEQ = a15.CALENDAR_DIM_SEQ)
        LEFT OUTER JOIN DARTEDM.D_PROV@DARTPRD a16
          ON ((CASE WHEN a11.SUPER_PROV_DIM_SEQ = -1
          THEN a11.PROV_DIM_SEQ
               ELSE a11.SUPER_PROV_DIM_SEQ END) = a16.PROV_DIM_SEQ)
        LEFT OUTER JOIN DARTEDM.MV_RVU_REPORT_CATEGORY@DARTPRD a17
  ...
  LEFT OUTER JOIN DARTEDM.MV_RVU_REPORT_CATEGORY@DARTPRD a12
  ...
  LEFT OUTER JOIN DARTEDM.D_CALENDAR@DARTPRD a13
  ...
  LEFT OUTER JOIN (SELECT
                   ...
                   FROM (SELECT
                           ...
                           EFFORT_PCT,
                           DISTRIB_PCT
                         FROM DARTEDM.MV_RVU_PROV_EFFORT@DARTPRD EFF
                           JOIN MICROSTRAT.DS_FN_DEPT_DISEASE_GRP_XREF@DARTPRD XREF
                             ON EFF.FN_DEPT_ID = XREF.FN_DEPT_ID) a11
                     LEFT OUTER JOIN DARTEDM.D_PROV@DARTPRD a12
                       ON (a11.PROV_DIM_SEQ = a12.PROV_DIM_SEQ)
                   WHERE a11.ACADEMIC_YR = (SELECT ACADEMIC_YR
                                            FROM DARTEDM.D_CALENDAR@DARTPRD
  ...
  LEFT OUTER JOIN DARTEDM.D_PROC@DARTPRD a15
    ON (pa11.SERVICE_CD = (CASE WHEN a15.PROC_CD IS NULL
    THEN 'N/A'
                           ELSE a15.PROC_CD END))
  LEFT OUTER JOIN DARTEDM.D_PROV@DARTPRD a16
    ON (pa11.PROV_ID0 = a16.EPIC_PROV_ID)
  LEFT OUTER JOIN DARTEDM.D_PROV@DARTPRD a17
    ON (pa11.PROV_ID = a17.EPIC_PROV_ID)
WHERE a16.EPIC_PROV_ID = '1003022'
      AND EXTRACT(YEAR FROM pa11.CALENDAR_DT) = 2017 AND EXTRACT(MONTH FROM pa11.CALENDAR_DT) = 12