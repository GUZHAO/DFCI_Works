...
print("Python Bit Version " + repr(struct.calcsize("P") * 8))
print()

# Import PROFILE population's MRN, using second tab
...
print("Sanity Check")
print("There are " + repr(MRN_profile['DFCI_MRN'].count()) + " records in Elia's file")  # Check row count -- passed
print("The MRN field length is " + repr(
    MRN_profile['DFCI_MRN'].str.len().unique()))  # Check whether MRN is only 6 digits -- passed

# Prepare output file parameters
time_str = time.strftime("%d-%m-%Y")
filename = "//cifs2/coba$/Ad-Hoc Data Requests/Elia's Profile Population/" + "PROFILE MRN" + time_str + ".csv"


def read_from_oracle(username, password, connectionstr, query):
    connection = cx_Oracle.connect(username, password, connectionstr)
    with connection:
        try:
            or_df = pandas.read_sql_query(query, connection)
            return or_df
        except cx_Oracle.DatabaseError as db_error:
            print(db_error)


# Get DFCI outpatient population
out_pop = read_from_oracle(username="COBATABLEAU", password="Bobby1234", connectionstr="dfcinp01-scan:1521/dartprdo",
                           query="""
                            SELECT
                              t1.DFCI_MRN,
                              t1.SITE,
                            ...
                            WHERE t1.ROW_NUM = 1
                           """)
EPSi_exam = read_from_oracle(username="COBATABLEAU", password="Bobby1234", connectionstr="dfcinp01-scan:1521/dartprdo",
                             query="""
                                SELECT
                                    t1.DFCI_MRN,
                                    t1.EXAM_FLAG,
                           ...
                           """)
EPSi_infu = read_from_oracle(username="COBATABLEAU", password="Bobby1234", connectionstr="dfcinp01-scan:1521/dartprdo",
                             query="""
                                SELECT
                                    t1.DFCI_MRN,
                                    t1.INFUSION_FLAG
                           ...
                           """)
# Add site and patient death date information
result = MRN_profile.set_index("DFCI_MRN").join(out_pop.set_index("DFCI_MRN")).join(EPSi_exam.set_index("DFCI_MRN")).join(EPSi_infu.set_index("DFCI_MRN"))
# export file
result.to_csv(filename)

# How many PROFILE MRNs are in outpatient population and how many are not
print(" ")
print("Here is the summary:")
out_pop_total = out_pop["DFCI_MRN"].count()
print("There are " + repr(out_pop_total) + " outpatients starting from EPIC go-live")
MRN_profile_miss = result["SITE"].isnull().sum()
print("There are " + repr(MRN_profile_miss) + " PROFILE MRNs that can not be found in outpatient population")
MRN_profile_not_miss = result["SITE"].count()
print("There are " + repr(MRN_profile_not_miss) + " PROFILE MRNs that can be found in outpatient population")
percentage = MRN_profile_not_miss / out_pop_total
print("The percentage of PROFILE MRNs against outpatient MRNs is " + "{:.2%}".format(percentage))
infu_percentage = (result["INFUSION_FLAG"] == "Y").sum() / MRN_profile["DFCI_MRN"].count()
print("The percentage of PROFILE MRNs who had infusion against outpatient MRNs is " + "{:.2%}".format(infu_percentage))
